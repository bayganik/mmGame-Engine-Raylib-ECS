...
namespace mmGameEngine
{
     ...
     * Rectangle that is used with View and World routines
     */
	public struct RectangleF : IEquatable<RectangleF>
	{
		...
		/// </summary>
		/// <value>The max.</value>
		public System.Numerics.Vector2 Max => new System.Numerics.Vector2(Right, Bottom);
		...
		{
			get => new System.Numerics.Vector2(X, Y);
			set
			{
				X = value.X;
				Y = value.Y;
		...
		{
			get => new System.Numerics.Vector2(Width, Height);
			set
			{
				Width = value.X;
				Height = value.Y;
		...
		//public float? RayIntersects(Ray2D ray)
		//{
		//	var num = 0f;
		//	var maxValue = float.MaxValue;
									...
									float rotation, float width, float height)
		{
			...
			else
			{
				...
				Location = new System.Numerics.Vector2(minX, minY);
				Width = maxX - minX;
				Height = maxY - minY;
			}
		}
		...
		public static implicit operator Rectangle(RectangleF self)
		{
			...
			Rectangle tmp = new Rectangle((int)self.X, (int)self.Y, (int)self.Width, (int)self.Height);
			return tmp;
		}
